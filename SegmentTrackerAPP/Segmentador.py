...
#home/mamendez/Datasets/DT_Estimations
#los pesos de la primera red
#los pesos del correo: la primera red
#los pesos del cenat: la segunda red
# MISC FUNCTIONS #
...
#se le pasan las dos redes
#ver en predictions 2          
def predict_images2(net, net2, device, loader, criterion, show=False, post="Softmax", dest=""):
    ...
    with torch.no_grad():
        for batch_idx, (data, gt, id) in enumerate(loader):
            ...
            # Forward
            predictions = net(data)
            predictions2 = net2(predictions)
            if post == "Softmax":
              predictions2 = F.softmax(predictions2)
            
            save_image(predictions2, dest+"/res_" + str(i) + '.png')
            #save_image(gt, 'results/gt_' + str(i) + '.png')
...
def predict2(load="checkpoints/Border/weights1.pth", load2="checkpoints/Border/weights1.pth", n_channels=1, n_classes=1, dir_pred="data/Original/Test/", dir_pred_2="predictions/DT/MAE/Test/", dir_gt="data/Distance_Transform/Test/", evaluation="MSE", out=None, dest="C:/a/"):
    ...
    net2 = UNet(n_channels=1, n_classes = 3).to(device)
    net2 = torch.nn.DataParallel(net2, device_ids=list(range(torch.cuda.device_count()))).to(device)
    #parametros de las dos redes
    #primera red: 1 1
    #segunda red: 1 3
    #net = Unet(n_classes = 1)
    #print("Load: ", load)
    ...
    if load:
        net.load_state_dict(torch.load(load, map_location="cpu")["state_dict"])
        net2.load_state_dict(torch.load(load2, map_location="cpu")["state_dict"])
        print('Model loaded from {}'.format(load))
        print('Model loaded from {}'.format(load2))
...
'''
"""
predict2(load="../weights/weights_red1.pth", #los pesos de la primera red
        load2="../weights/weights_red2.pth",#los pesos de la segunda red
        n_channels = 1,
        n_classes = 3,
